...
%
A canonical example of this kind arises in the context of
typechecking.
%
This is an analysis that is built into determining the acceptability
of a program before it is run.
%
...
asked to be performed on data to which it cannot be meaningfully
applied.
%
...
%
That process must have as input also a specification of the execution
...
%
First, there are the language specifications, which encompass those
describing analyses (such as type-checking) and those specifying
behavior (such as the execution steps which are based on the program
expression). 
%
...
%
The difficulties in the library-based view of extensible languages are
twofold.
%
...
across different components and, in any case, this task has to be
carried out with only partial knowledge of the language constructs and
associated specifications.

In their recent collaborative work, the PIs have pioneered an effort
to come to grips with the metatheory problem for extensible languages~\cite{michaelson2025toplas}.
%
A key idea originating from their work is that the host language can
...
language and they are ones whose proof each extension can contribute
to.
%
An auxiliary property is oriented around an analysis introduced by an
extension and it is therefore one unknown to other components.
%
The proof of an auxiliary property must therefore be constructed
entirely within the extension that introduces it but, of course, using
its knowledge of the host language and projection constraints.
%
...
called Extensibella, thereby providing a vehicle for experimentation
...
%
This project will make a start in this direction.
%
Two particular questions will be targeted in its duration:
%
\begin{enumerate}
...
\item Our approach to treating interactions between extensions
  ...
  %
  At a formal level, this accords well with the motto in programming
  language work of obtaining dependability through discipline.
  %
  ...
  explored. 
  %
  We will conduct experiments, perhaps use some of the extensibility
  libraries developed by the Van Wyk group, towards understanding the
  choices in these realms better.
\end{enumerate}
%
The work in both cases will be carried out by understanding proof
methods, identifying language libraries that will provide insights
into the underlying questions, constructing models and proofs, and
analyzing aspects such as the extensions that can be described and the
properties that can be expressed and proved within the framework.
%
The Extensibella system will be used in the experiments.
%
The primary effort will be that of a graduate student with interest in
programming languages research.
%
Both PIs will oversee the effort.

While the work that we intend to carry out has its own intellectual
justification, we mention that the questions that it will investigate
were raised about an NSF proposal for which funding was declined in
2025 despite a competitive rating.
%
We have tried to respond to these questions in a different way in a
recent resubmission.
%
The successful completion of this project will enable a more direct
response to what we see as the main obstacle to NSF funding for this
