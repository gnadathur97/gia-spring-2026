...
\noindent Programming languages play a fundamental role in the
permeation of computing into everyday applications. 
%
Research on such languages is devoted to understanding aspects of
their structure that makes them effective in this domain.
%
Such research has two complementary foci: \emph{expressivity} and
\emph{verifiability}.
%
Expressivity aims to give substance to the sentiment that languages
should make it easy to translate conceived solutions to computational
problems into actual, working programs.
%
Verifiability, on the other hand, dictates that the languages we
design should make it easier to ascertain that the programs we write
in fact realize the intended computations.
%
...
%
Modern-day applications have raised another challenge for the notion
...
investigated as a means for supporting expressivity in this context.
%
While such research has been largely successful in enhancing
expressivity, it raises new questions with regard to verifiability.
%
The work we propose here fits under a broader project that the PIs are
...
verifiability in more detail below towards providing a framework for
describing the specific work we will undertake. 

\medskip
\noindent {\bf Language expressivity and extensibility.} Expressivity
had for long been considered to be an attribute of one properly
designed language.  
%
However, this view has been challenged by the realization that the
meaning of expressivity can be impacted by aspects such as the
granularity of computations and the domain to which they are applied.
%
...
language extensibility: the questions that are asked pertain to the
properties that the library should possess to make compositions work
and to give programmers a seamless view of a language that ``under the 
covers'' may actually be the result of combining different components
from a library. 

\medskip
\noindent {\bf Language extensibility and modularity.} A fundamental
requirement of the library view of extensibility is that language
extensions must be {\it composable}, \ie, it should be possible to use
an extension in combination with \emph{any collection} of other
extensions from the library to produce a working and well-behaved
language.  
%
...
%
This issue has been addressed by the work of several research groups,
including the group led by one of the PIs.
%
...
library view of language extensibility can be assumed to be a solved
problem, as we do within this project. 
...
aspects of the scheme that seems important to securing funds from the
NSF for a more ambitious investigation. 
